#!/bin/bash
set -e

source common/ui.sh

export VAGRANT_KEY="ssh-rsa  vagrant insecure public key"

info "Preparing vagrant user..."

# Configure SSH access
mkdir -p ${ROOTFS}/home/ubuntu/.ssh
echo $VAGRANT_KEY > ${ROOTFS}/home/ubuntu/.ssh/authorized_keys
chroot ${ROOTFS} chown -R ubuntu: /home/ubuntu/.ssh
log 'SSH credentials configured for the ubuntu user.'

echo "ubuntu ALL=(ALL) NOPASSWD:ALL" > ${ROOTFS}/etc/sudoers.d/ubuntu
chmod 0440 ${ROOTFS}/etc/sudoers.d/ubuntu
